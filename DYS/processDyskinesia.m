...
%%% EVALUATION OF DYSKINESIA INTENSITY ESTIMATOR
...
for jj=1:size(regions,1)    
    ...
    [p]=estimateDyskinesia(time(regions(jj,1):regions(jj,2))*1000,data(regions(jj,1):regions(jj,2),:));    
    tremorEst=[tremorEst;p];
    
    tremorInt=[tremorInt;(jj-1)*ones(size(p,1),1)];
end
...
time=posixtime(d.VarName1);
data=[d.VarName2 d.VarName3 d.VarName4];
regions=[600 1300;1000 4000;5000 6700];
dysEst=[];
dysInt=[];
dysP=zeros(size(regions,1),1);
%%
for jj=1:size(regions,1)    
    [p]=estimateDyskinesia(time(regions(jj,1):regions(jj,2))*1000,data(regions(jj,1):regions(jj,2),:));
    ...
    dysInt=[dysInt;(jj-1)*ones(size(p,1),1)];
    
    dysP(jj,1)=sum(p>70)/length(p);
end
%%


p1=tremorEst(find(tremorInt==0));
p2=tremorEst(find(tremorInt>0));

d1=dysEst(find(dysInt==0));
d2=dysEst(find(dysInt>0));



C=[ones(size(p1,1),1);ones(size(p2,1),1);ones(size(d1,1),1);2*ones(size(d2,1),1)];
