...
%excluded=[79 148];
    ...
    for ff=1:length(files)
        ...
        if(exist(files{ff}))
            ...
            [leftSwing,rightSwing]=analyzeIMU(d1);
              [leftSwingAnnot,rightSwingAnnot]=analyzeInsoles2(d1,3);
    ...
    fprintf('Average  %f %f \n',mean(results(:,end-1)),mean(results(:,end)));
...
%%
ii=find(results(:,6)>10&results(:,6)<50&results(:,14)<2);
BlandAltman(results(ii,5),results(ii,6),{'INSOLES','IMU'})
swingInsoles=results(ii,9);
swingIMU=results(ii,10);
b=robustfit(swingIMU,swingInsoles);
swingIMUhat=b(2)*swingIMU+b(1);
BlandAltman(swingInsoles,swingIMUhat,{'INSOLES','IMU'})



stInsoles=results(ii,14);
stIMU=results(ii,13);
b=robustfit(stIMU,stInsoles);
stIMUhat=b(2)*stIMU+b(1);
BlandAltman(stInsoles,stIMUhat,{'INSOLES','IMU'})



dsInsoles=results(ii,18);
dsIMU=results(ii,17);
b=robustfit(dsIMU,dsInsoles);
dsIMUhat=b(2)*dsIMU+b(1);
BlandAltman(dsInsoles,dsIMUhat,{'INSOLES','IMU'})
...
[a b]=ttest2(res1(pdSubject1==1,:),res1(pdSubject1==0,:))
figure,
boxplot(res1(:,end),pdSubject1);
