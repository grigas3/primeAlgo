function [psdEnergies mEnergies]=estimateTremor(time,d)
...
for jj=1:L
    ...
    w1=sigmf(e1/(e1+e2),[10 0.5]); %%% Weight
    
    m1=max(psdxT(findex1))/mean(psdxT(findex1));
    w2=(sigmf(m1,[2 2.5]))
    psdEnergies(jj)=w1*w2*e1+0.00001; %%% Total Tremor Index
    mEnergies(jj)=m1;
    if(dispPlot)
